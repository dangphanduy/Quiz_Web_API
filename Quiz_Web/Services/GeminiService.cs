...
public class GeminiService : IGeminiService
{
    ...
    public async Task<string> GetResponseAsync(string prompt, string context)
    {
        try
        {
            ...
            var systemInstruction = "Bạn là AI Course Assistant - Trợ lý tư vấn khóa học thông minh của hệ thống. " +
            ...
            var requestBody = new
            {
                contents = new[]
                {
                    new
                    {
                        ...
                        parts = new[]
                        {
                            new { text = $"[Bối cảnh / Ngữ cảnh các khóa học có sẵn]:\n{context}\n\n[Câu hỏi từ người dùng]:\n{prompt}" }
                        }
                    }
                },
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogError($"Gemini API error: {response.StatusCode} - {errorMsg}");
